...
public class InfoManager : MonoBehaviour
{
    ...
    public void UpdateView()
    {
        if (viewInput.text != "")
        {
            ...
            {
                photonView.RPC("UpdateViewRPC", RpcTarget.AllBuffered, viewInput.text, photonView.ViewID);
            }
        }
    }
    ...
    public void UpdateHealth()
    {
        if (healthInput.text != "")
        {
            if (int.Parse(healthInput.text) >= 0)
            {
                photonView.RPC("UpdateHealthRPC", RpcTarget.AllBuffered, healthInput.text, photonView.ViewID);
            }
        }
    }
